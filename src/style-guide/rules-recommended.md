...
**앨리먼트(컴포넌트 포함)의 속성은 일관된 순서로 정렬해야 합니다.**
...
2. **리스트 렌더링** (동일한 앨리먼트의 여러 변형을 생성합니다.)

   - `v-for`

3. **조건부** (앨리먼트 렌더링/표시 여부)
...
4. **렌더링 수정자** (앨리먼트의 렌더링 방식을 변경합니다.)

   - `v-pre`
   - `v-once`

5. **글로벌 인식** (컴포넌트 이상의 지식이 필요합니다.)
