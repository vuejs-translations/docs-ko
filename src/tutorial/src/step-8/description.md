...
이전 단계의 할 일 목록을 계속 확장해 봅시다. 여기서는 이미 각 할 일에 토글 기능을 추가했습니다. 이는 각 할 일 객체에 `done` 속성을 추가하고, 이를 체크박스에 `v-model`로 바인딩(binding)하여 구현합니다:
...
다음으로 추가할 개선점은 이미 완료된 할 일을 숨길 수 있는 기능입니다. 우리는 이미 `hideCompleted` 상태를 토글하는 버튼을 가지고 있습니다. 하지만 그 상태에 따라 어떻게 다른 목록 항목을 렌더링(rendering)할 수 있을까요?
