...
때때로 우리는 "부수 효과(side effects)"를 반응적으로 수행해야 할 필요가 있습니다. 예를 들어, 숫자가 변경될 때마다 콘솔에 로그를 남기는 경우가 있습니다. 이러한 작업은 감시자(watcher)를 통해 달성할 수 있습니다:
...
여기서는 `watch` 옵션을 사용하여 `count` 속성의 변화를 감시하고 있습니다. watch 콜백은 `count`가 변경될 때 호출되며, 새로운 값이 인자로 전달됩니다. 자세한 내용은 <a target="_blank" href="/guide/essentials/watchers.html">가이드 - 감시자</a>에서 확인할 수 있습니다.
