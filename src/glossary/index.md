...
*커스텀 엘리먼트(Custom Element)*는 현대적인 웹 브라우저에서 구현된 [웹 컴포넌트(Web Components)](#web-components) 표준의 기능입니다. 이는 HTML 마크업에서 커스텀 HTML 엘리먼트를 사용하여 해당 위치에서 웹 컴포넌트를 포함시킬 수 있는 능력을 의미합니다.
...
자세한 내용은 다음을 참조하세요:
- [가이드 - 깊이 있는 반응성](https://vuejs.org/guide/extras/reactivity-in-depth.html)
...
스케줄러의 작업은 [라이프사이클 훅](#lifecycle-hooks)을 트리거하거나 [템플릿 ref](#template-refs)를 업데이트하는 등 다양한 내부 작업에도 사용됩니다.

## 스코프드 슬롯 (Scoped Slot) {#scoped-slot}

"스코프드 슬롯(scoped slot)"이라는 용어는 [슬롯(slot)](#slot)이 [프롭(props)](#props)을 받는 것을 가리킵니다.
