...
본 개인정보 보호정책은 Vue.js 조직("Vue", "우리", "당사")이 이 웹사이트(https://vuejs.org)와 우리의 오픈 소스 관련 웹사이트("웹사이트") 및 본 웹사이트에서 제공하는 모든 콘텐츠, 관련 문서, 정보 및 서비스(예: 튜토리얼, 개발자 워크플로우 지원 도구, 리소스 접근 등)(통칭하여 "서비스")에 대해 귀하의 정보를 처리하는 방식을 설명합니다. 본 개인정보 보호정책은 당사의 서비스를 지원하기 위해 처리하는 개인 정보에 대해 설명합니다.
...
4. 브라우저 / IDE 벤더(vendor)가 수집한 당사의 게시된 브라우저 / IDE 확장의 사용 통계. 이러한 통계는 벤더의 해당 약관에 의해 관리됩니다.

## 어떤 종류의 정보를 수집하나요? {#what-kinds-of-information-do-we-collect}

우리는 웹사이트나 게시된 npm 패키지 또는 브라우저 / IDE 확장을 통해 어떤 종류의 개인 데이터도 **수집하거나 저장하지 않습니다**.
...
앞서 언급한 데이터를 수집하는 유일한 목적은 가능한 한 개인정보 친화적인 방식으로 당사 웹사이트 트래픽과 사용을 이해하여 웹사이트와 문서의 품질을 지속적으로 개선하기 위함입니다. GDPR에 따른 합법적 근거는 "제6조(1)(f); 당사의 정당한 이익은 웹사이트와 비즈니스를 지속적으로 개선하는 것입니다." 앞서 설명한 바와 같이, 개인 데이터는 지속적으로 저장되지 않습니다.
