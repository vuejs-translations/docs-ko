...
저희의 [행동 강령](/about/coc)은 우리 모두와 우리가 참여하는 기술 커뮤니티를 더욱 풍요롭게 만들기 위한 가이드입니다.

### 최신 정보 파악 {#stay-in-the-know}

- [공식 트위터 계정](https://x.com/vuejs)을 팔로우하세요.
- 트위터 또는 GitHub에서 [팀원](./team)을 팔로우하세요.
...
- [밋업](https://events.vuejs.org/meetups): 여러분과 같은 Vue 애호가를 가까운 지역에서 찾고 싶으신가요? 커뮤니티 리더가 되고 싶으신가요? 필요한 도움과 지원이 바로 여기에 있습니다!
- [GitHub](https://github.com/vuejs): 보고할 버그나 요청할 기능이 있는 경우 GitHub 이슈를 이용하세요. 각 리포지토리의 이슈 템플릿(template)에 명시된 규칙을 준수해 주세요.
...
Vue 커뮤니티에 기여하는 방법이 코드 기여만 있는 것은 아닙니다. Discord 또는 포럼에서 동료 Vue 사용자의 질문에 답변하는 것도 가치 있는 기여로 간주됩니다.

### 이슈 분류 돕기 {#help-triage-issues}

이슈를 분류한다는 것은 누락된 정보를 수집하고, 버그를 재현해 보고, 이슈가 유효한지 확인하고, 그 원인을 조사하는 것을 의미합니다.

저희는 매일 [GitHub의 리포지토리](https://github.com/vuejs)에 많은 이슈를 접수합니다. 사용자 수에 비해 팀의 여력이 제한되어 있기 때문에 이슈를 분류하는 것만으로도 팀에서 엄청난 노력을 기울여야 합니다. 이슈를 분류하는 작업을 도와주면 효율성이 향상되어 더 우선순위가 높은 작업에 시간을 할애할 수 있습니다.

이슈를 해결하기 위한 목적으로 분류할 필요는 없습니다(물론 그것도 좋습니다). 버그를 유발한 커밋과 같은 조사 결과를 공유하는 것만으로도 이미 많은 시간을 절약할 수 있습니다.
...
포럼과 채팅에서 질문에 답변하고 리소스를 공유하는 것 외에도, 여러분이 알고 있는 것을 공유하고 확장할 수 있는, 눈에 잘 띄지 않는 방법도 몇 가지 있습니다:

- **학습 자료 개발하기.** 가장 좋은 학습 방법은 가르치는 것이라고 흔히들 말합니다. Vue로 흥미로운 작업을 하고 있다면 블로그 게시물을 작성하거나 워크샵을 개발하거나 소셜 미디어에 공유할 gist를 게시하여 전문성을 강화하세요.
- **관심 있는 리포지토리 지켜보기(watch).** 리포지토리를 지켜보면 활동이 있을 때마다 알림을 받게 되어, 진행 중인 토론과 향후 기능에 대한 내부 정보를 얻을 수 있습니다. 전문 지식을 쌓아 궁극적으로 이슈와 풀 리퀘스트를 해결하는 데 도움을 줄 수 있는 환상적인 방법입니다.

### 문서 번역 {#translate-docs}

지금 이 문장을 선호하는 언어로 읽고 계시길 바랍니다. 그렇지 않다면 저희를 도와주시겠어요? 

[번역 가이드](/translations/)에서 참여 방법에 대한 자세한 내용을 확인할 수 있습니다.
...
- **나만의 모임 시작하기.** 해당 지역에 아직 Vue 모임이 없는 경우 직접 모임을 시작할 수 있습니다! [이벤트를 위한 리소스](https://events.vuejs.org/resources/#getting-started)를 활용하면 성공적으로 시작하는 데 도움이 됩니다!
- **밋업 주최자 돕기.** 이벤트를 운영하는 데 도움은 아무리 많이 받아도 지나치지 않으므로, 모든 이벤트가 성공할 수 있도록 지역 주최자를 도와주세요.

지역 Vue 커뮤니티에 더 많이 참여할 수 있는 방법에 대해 궁금한 점이 있으면 트위터 [@vuejs_events](https://x.com/vuejs_events) 로 문의하세요!
