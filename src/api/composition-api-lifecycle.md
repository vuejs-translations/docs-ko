...
- **상세 설명**
  ...
  **오류 전파 규칙**
  ...
  - 기본적으로, 애플리케이션 레벨의 [`app.config.errorHandler`](/api/application#app-config-errorhandler)가 정의되어 있다면 모든 오류는 여전히 해당 핸들러로 전달됩니다. 이를 통해 오류를 한 곳에서 분석 서비스로 보고할 수 있습니다.
