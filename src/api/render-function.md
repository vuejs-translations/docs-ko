...
- **세부사항**
  ...
  컴포넌트 vnode를 생성할 때, 자식은 슬롯(slot) 함수로 전달되어야 합니다. 컴포넌트가 기본 슬롯만 기대한다면 단일 슬롯 함수를 전달할 수 있습니다. 그렇지 않으면 슬롯을 슬롯 함수 객체로 전달해야 합니다.
...
등록된 디렉티브(directive)를 이름으로 수동으로 해석할 때 사용합니다.
