...
## 폴백(fallback) 콘텐츠 {#fallback-content}
...
header / footer / default에 대한 콘텐츠가 있을 때, 추가 스타일링을 제공하기 위해 해당 콘텐츠를 래핑하고자 합니다:
...
실제로 그렇게 할 수 있습니다. 컴포넌트에 props를 전달하듯이, 슬롯 아웃렛에도 속성을 전달할 수 있습니다. 부모 템플릿은 `v-slot`으로 슬롯 props를 받고, 자식 템플릿은 렌더링할 때 슬롯 아웃렛에 props를 전달합니다:
...
실제로, 이것은 스코프드 슬롯이 컴파일되는 방식이나 수동 [렌더 함수](/guide/extras/render-function)에서 스코프드 슬롯을 사용하는 방식과 매우 유사합니다.
...
### 명명된 스코프드 슬롯 {#named-scoped-slots}
...
흥미로운 패턴이긴 하지만, 렌더리스 컴포넌트로 달성할 수 있는 대부분의 기능은 컴포지션 API를 사용하면 더 효율적으로 구현할 수 있으며, 불필요한 컴포넌트 중첩의 오버헤드도 피할 수 있습니다. 이후에 동일한 마우스 추적 기능을 [컴포저블(composable)](/guide/reusability/composables)로 구현하는 방법을 살펴볼 것입니다.
