...
HTML의 내장 입력 타입만으로 요구사항을 항상 충족할 수 있는 것은 아닙니다. 다행히도 Vue 컴포넌트를 사용하면 동작을 완전히 커스텀한 재사용 가능한 입력을 만들 수 있습니다. 이러한 입력도 `v-model`과 함께 동작합니다! 더 자세한 내용은 컴포넌트 가이드의 [v-model과 함께 사용하기](/guide/components/v-model)를 참고하세요.
