...
버튼을 클릭할 때마다 각 버튼이 자신의 `count` 값을 별도로 유지하는 것을 알 수 있습니다. 이는 컴포넌트를 사용할 때마다 새로운 **인스턴스(instance)**가 생성되기 때문입니다.
...
그런 다음, `v-for`를 사용해 각 포스트마다 컴포넌트를 렌더링(rendering)하고 싶을 것입니다:
...
## 슬롯(slot)을 이용한 콘텐츠 분배 {#content-distribution-with-slots}
