...
자식 컴포넌트를 사용하려면 부모 컴포넌트에서 가져와야(import) 합니다.
...
기본 HTML 태그 이름은 대소문자를 구분하지 않지만, Vue의 SFC는 컴파일된 포맷으로 대소문자를 구분하여 태그 이름을 사용할 수 있습니다.
...
사용자 정의 컴포넌트 `<blog-post-row>`는 잘못된 컨텐츠로 호이스트(hoisted)되어 최종적으로 렌더링된 출력에서 에러를 발생시킵니다.
...
재미있는 것을 빌드하거나 아직 확인하지 않았다면 몇 가지 [예제](/examples/)를 확인해보세요.
