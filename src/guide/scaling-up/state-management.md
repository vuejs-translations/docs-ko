...
이 컴포넌트는 다음과 같은 부분으로 이루어진 독립적인 단위입니다:

- **상태**: 앱을 구동하는 진실의 원천
- **뷰**: **상태**의 선언적 매핑
- **액션**: **뷰**에서 사용자 입력에 반응하여 상태가 변경되는 방법
...
더 간단하고 직관적인 해결책은 공유 상태를 컴포넌트에서 분리하여 전역 싱글턴에서 관리하는 것입니다. 이렇게 하면 컴포넌트 트리는 하나의 큰 "뷰"가 되고, 트리 어디에 있든 모든 컴포넌트가 상태에 접근하거나 액션을 트리거할 수 있습니다!
...
이제 `store` 객체가 변경될 때마다 `<ComponentA>`와 `<ComponentB>` 모두 자동으로 뷰가 갱신됩니다. 하나의 진실의 원천이 생긴 것입니다.
