...
버튼을 클릭해보면 숫자가 변하지 않는 것을 알 수 있습니다. 브라우저에서 Vue를 로드하지 않았기 때문에, 클라이언트의 HTML은 완전히 정적인 상태입니다.
...
이 파일과 그 의존성들은 서버와 클라이언트에서 모두 공유됩니다. 이를 **유니버설 코드**라고 부릅니다. 유니버설 코드를 작성할 때 주의해야 할 점들이 있는데, 이는 [아래에서 더 자세히 다룹니다](#writing-ssr-friendly-code).
...
상태 관리 챕터에서는 [반응성 API를 이용한 간단한 상태 관리 패턴](state-management#simple-state-management-with-reactivity-api)을 소개했습니다. SSR 환경에서는 이 패턴에 추가적인 조정이 필요합니다.
