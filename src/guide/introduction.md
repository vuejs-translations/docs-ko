...
Vue(발음: /vjuː/, **view**와 비슷함)는 사용자 인터페이스를 구축하기 위한 자바스크립트 프레임워크입니다. 표준 HTML, CSS, JavaScript 위에 구축되며, 선언적이고 컴포넌트(component) 기반의 프로그래밍 모델을 제공하여 복잡도에 상관없이 효율적으로 사용자 인터페이스를 개발할 수 있도록 도와줍니다.
...
옵션 API에서는 `data`, `methods`, `mounted`와 같은 옵션 객체를 사용하여 컴포넌트의 로직을 정의합니다. 옵션으로 정의된 속성들은 함수 내부에서 `this`로 노출되며, 이는 컴포넌트 인스턴스(instance)를 가리킵니다:
...
컴포지션 API에서는 가져온 API 함수들을 사용하여 컴포넌트의 로직을 정의합니다. SFC에서는 컴포지션 API를 [`<script setup>`](/api/sfc-script-setup)과 함께 사용하는 것이 일반적입니다. `setup` 속성은 Vue가 컴파일 타임 변환을 수행하도록 힌트를 주어, 컴포지션 API를 더 적은 보일러플레이트(boilerplate)로 사용할 수 있게 해줍니다. 예를 들어, `<script setup>`에서 선언된 import와 최상위 변수/함수는 템플릿에서 바로 사용할 수 있습니다.
